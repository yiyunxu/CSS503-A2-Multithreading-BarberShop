...
public:
  ...
  void setState(customer_state state);
  customer_state getState();
...
void Customer::setState(customer_state state){
  this->state = state;
}

Customer::customer_state Customer::getState(){
  return state;
}

pthread_mutex_t* Customer::getMutex(){
  return c_mutex;
}

pthread_cond_t* Customer::getCondition(){
  return c_condition;
}
...
void Shop::run() {
  ...
  for (int next_customer_id = 0; ; next_customer_id++) {
    ...
    pthread_attr_t* detachedThread = reinterpret_cast<pthread_attr_t*>(malloc(sizeof(pthread_attr_t)));
    pthread_attr_init(detachedThread);
    pthread_attr_setdetachstate(detachedThread, PTHREAD_CREATE_DETACHED);

    int rc2 = pthread_create(thread, detachedThread,
  ...
  while (true){
    {
      ...
      while (state != barber_state::AWAKE){
        ...
        if (closing &&  customer->getState() == Customer::customer_state::LEAVE){
    ...
    Customer* nextCust = shop->next_customer(this);
      if (nextCust == nullptr){  //barber go to sleep 
        ...
        {
          Lock lock(nextCust->getMutex());
          nextCust->setBarber(this);
          nextCust->setState(Customer::customer_state::GOTOBARBER);
          pthread_cond_signal(nextCust->getCondition());
        }
